...
- `init_window.rs` lifecycle guard (requires passing prev Window account → IDL change). Demo workflow depends on permissionless `init_window` to advance state past stuck Aggregating windows, so adding the guard requires also wiring a workflow alternative. Punted post-submission.
...
- ~~Refund instruction~~ — **DEPLOYED** in 4th upgrade tx [`23f5YfeR...`](https://explorer.solana.com/tx/23f5YfeRK44cSUBhnmZe5mWV9CdL4gHXaAtT9p8UZHi3SiC5HQSnaGDNMqzpWUe524tpqduwugBcA7ZWrh2s3UCr?cluster=devnet) — adds 2 new instructions: `mark_window_failed` (pool authority moves Aggregating → Failed) + `refund_intent` (user pulls their commit back from a Failed window's escrow). Tightened `claim_allocation` to status==2 only so refund-eligible users can't accidentally claim 0 tokens. End-to-end validated on devnet:
  - `trigger_aggregate`: [`67fBCQyG...`](https://explorer.solana.com/tx/67fBCQyG33dc3NyXQFhpXEkxCQLEbyjrsk91AQPQUEEQBrehp1iaa8eVuciygpLhRuNB6J5BtXRewsTZnDCytkYb?cluster=devnet) (Window #6: Open → Aggregating)
  ...
  - `refund_intent`: [`SDrdCnJ3...`](https://explorer.solana.com/tx/SDrdCnJ3HBHLqUAUkYmFTkMUBKjoH2BC6eSetZnpGZD2XVpiR1UJaZv5KrrH1671SQWDyudKYJnbfFZCU1Z7q5k?cluster=devnet) (wallet +$10 USDC, exactly intent.amount). Failed state is now reachable AND recoverable end-to-end.

**Program instruction count**: 7 → **9** (added `mark_window_failed`, `refund_intent`).
